...
public class MyCombinedChart extends CombinedChart {
    ...
    protected void drawMarkers(Canvas canvas) {
        if (!mDrawMarkerViews || !valuesToHighlight()){
            if (null != hightLightSelect) {
                hightLightSelect.isHightLightShow(false);
            }
            return;
        }

        if (null != hightLightSelect) {
            hightLightSelect.isHightLightShow(true);
        }
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (xIndex <= deltaX && xIndex <= deltaX * mAnimator.getPhaseX()) {
                ...
                if (null != myMarkerViewH) {
                    ...
                    myMarkerViewH.draw(canvas, mViewPortHandler.contentLeft(), pos[1] - myMarkerViewH.getHeight() / 2);
                }
    ...
    private OnHightLightSelect hightLightSelect;

    public OnHightLightSelect getHightLightSelect() {
        return hightLightSelect;
    }

    public void setHightLightSelect(OnHightLightSelect hightLightSelect) {
        this.hightLightSelect = hightLightSelect;
    }

    public static interface OnHightLightSelect{
       void isHightLightShow(boolean isShow);
    }
}
