...
public class NeoTransferAccountsActivity extends BaseActivity {
    ...
    protected void initView() {
        ...
        tvCurrentPrice.setText("(" + getString(R.string.dangqianyue) + "：" + (type == 0 ? new BigDecimal(neoBean.getBalance()).setScale(0, BigDecimal.ROUND_HALF_UP).toPlainString() : new BigDecimal(neoBean.getGnt().get(0).getBalance()).setScale(8, BigDecimal.ROUND_DOWN).toPlainString()) + ")");
                        ...
                        if (null != response) {
                            if (!isClod){
                                if (null != response.body().data.getResult()) {
                                    ...
                                    intent.putExtra("price", new BigDecimal(etPrice.getText().toString().trim()).setScale(8,BigDecimal.ROUND_DOWN).toPlainString());
